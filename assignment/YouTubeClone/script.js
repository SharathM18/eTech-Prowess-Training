...
searchInput.addEventListener("keydown", async function (event) {
  if (event.code === "Enter") {
    try {
      ...
      let res = await fetch(
        `https://youtube.googleapis.com/youtube/v3/search?part=snippet&q=${inp}&key=AIzaSyCWmsjslHkf5HrCvKjkSL-G89v3inCk-18&maxResults=40&order=viewCount&safeSearch=strict`
      );
...
const videos = async () => {
  try {
    let res = await fetch(
      `https://youtube.googleapis.com/youtube/v3/search?part=snippet&type=video&key=AIzaSyCWmsjslHkf5HrCvKjkSL-G89v3inCk-18&maxResults=80&order=viewCount&safeSearch=moderate`
    );
    let data = await res.json();
    console.log(data.items);
    append(data.items);
